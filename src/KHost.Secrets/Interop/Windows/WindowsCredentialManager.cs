...
//     src/Core/Interop/Windows/WindowsCredentialManager.cs  @ 838e3496
//
...
//
// Changed from upstream: nullable disabled (upstream is unannotated), namespace; argument guard inlined, platform guard inlined.
...
using System.Text;
using KHost.Secrets.Interop.Windows.Native;
...
namespace KHost.Secrets.Interop.Windows
{
    public class WindowsCredentialManager : ICredentialStore
    {
        ...
        {
            if (!OperatingSystem.IsWindows()) throw new PlatformNotSupportedException("This store is Windows only.");
