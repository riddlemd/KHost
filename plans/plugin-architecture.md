...
provider shipped; Example still to come). The rotation engine is ported, registered,
...
Last remaining piece: the Example provider plugin.

## Goals

- Drop-in plugins: a folder under `plugins/`, picked up at app restart. No hot load/unload.
- Extension points for v1: **media search providers** (YouTube, Example ship this way) and
...
3. **First provider plugin (YouTube)** — proves the SDK ergonomics end to end; Example follows.
