...
- A plugin that needs to **ask the host for a value** — a login, a room code, anything it has no
  ...
  puts it in the secret store with `IPluginContext.SetSecretAsync`. `KHost.Plugins.KaraFun`'s
  ...
  it reaches the hashing call and nothing else, which is also what goes to KaraFun. Mark a field
...
- A plugin that owns media it must not let out without a live entitlement implements
  ...
  - **Ownership and verdict are separate questions.** Ownership is the tag: a gated file carries the
    ...
    writes `khost_provider=KHost.Plugins.KaraFun` with ffmpeg's `+use_metadata_tags` movflag (a custom
  ...
  - **`Claims(path)` is the fallback for a format that cannot carry a tag, and it closed a real
    ...
    KaraFun library through signed out. `Claims` answers from the **path alone** — it is asked for
