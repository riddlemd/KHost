...
- A plugin that renders media it must not let play without a live entitlement implements
  ...
  (`khost_gate`) set to its own `GateKey`, and `PlaybackService.LoadAsync` asks the matching gate
  before every load. KaraFun writes `khost_gate=karafun` into the mp4 (with ffmpeg's
