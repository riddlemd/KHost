...
public class MediaTagReaderTests : IDisposable
{
    ...
    public async Task ReadTag_FindsTheMarkerWhateverCaseItIsAskedFor()
    {
        ...
        Assert.Equal("KHost.Plugins.KaraFun", await _reader.ReadTagAsync(path, "KHOST_PROVIDER"));
        Assert.Equal("KHost.Plugins.KaraFun", await _reader.ReadTagAsync(path, "Khost_Provider"));
    }
    ...
    // A .khv holding mp4 bytes, exactly what KaraFun writes: obscuring extension, muxer forced
