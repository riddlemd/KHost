...
public class UsersServiceTests
{
    ...
    public async Task CreateAsync_WritesTheForeignKeysItWasHanded()
    {
        ...
        {
            Name = "Ada",
            ForeignKeys = [new KHostUserForeignKey { Source = "Example", Key = "remote-1", IsEphemeral = true }],
        };
        ...
        await _repository.Received(1).AddForeignKeyAsync(user.Id, "Example", "remote-1", true);
    ...
    public async Task ReadByForeignKeyAsync_AsksTheRepositoryForThatPair()
    {
        ...
        _repository.ReadByForeignKeyAsync("Example", "remote-1").Returns(ada);

        Assert.Same(ada, await _service.ReadByForeignKeyAsync("Example", "remote-1"));
    }
    ...
    public async Task DeleteEphemeralForeignKeysAsync_AnnouncesOnlyWhenSomethingWent()
    {
        ...
        _repository.DeleteEphemeralForeignKeysAsync("Example").Returns(0);
        await _service.DeleteEphemeralForeignKeysAsync("Example");
        Assert.Equal(0, raised);

        _repository.DeleteEphemeralForeignKeysAsync("Example").Returns(2);
        Assert.Equal(2, await _service.DeleteEphemeralForeignKeysAsync("Example"));
