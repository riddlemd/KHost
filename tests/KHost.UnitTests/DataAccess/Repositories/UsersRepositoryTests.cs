...
public class UsersRepositoryTests : IDisposable
{
    ...
    public async Task ReadByForeignKey_FindsTheSingerThatKeyNames()
    {
        ...
        await _repository.AddForeignKeyAsync(ada.Id, "KaraFun", "remote-1", isEphemeral: true);

        Assert.Equal("Ada", (await _repository.ReadByForeignKeyAsync("KaraFun", "remote-1"))?.Name);
        Assert.Null(await _repository.ReadByForeignKeyAsync("KaraFun", "remote-2"));
    }
    ...
    public async Task ReadByForeignKey_TellsTwoSourcesApartOnTheSameKey()
    {
        ...
        await _repository.AddForeignKeyAsync(ada.Id, "KaraFun", "42", isEphemeral: false);
        await _repository.AddForeignKeyAsync(grace.Id, "YouTube", "42", isEphemeral: false);

        Assert.Equal("Ada", (await _repository.ReadByForeignKeyAsync("KaraFun", "42"))?.Name);
    ...
    public async Task ReadByForeignKey_MatchesExactlyRatherThanFolded()
    {
        ...
        await _repository.AddForeignKeyAsync(ada.Id, "KaraFun", "ABC", isEphemeral: false);

        Assert.NotNull(await _repository.ReadByForeignKeyAsync("KaraFun", "ABC"));
        Assert.Null(await _repository.ReadByForeignKeyAsync("KaraFun", "abc"));
    }
    ...
    public async Task AddForeignKey_RefusesOneAnotherSingerAlreadyHolds()
    {
        ...
        var grace = User("Grace");
        await _database.SeedAsync(ada, grace);
        await _repository.AddForeignKeyAsync(ada.Id, "KaraFun", "remote-1", isEphemeral: false);

        var taken = await Assert.ThrowsAsync<KHostException>(
            () => _repository.AddForeignKeyAsync(grace.Id, "KaraFun", "remote-1", isEphemeral: false));
    ...
    public async Task Read_BringsTheForeignKeysWithIt()
    {
        ...
        Assert.Equal("KaraFun", key.Source);
    ...
    public async Task DeleteEphemeralForeignKeys_LeavesDurableOnesAndOtherSourcesAlone()
    {
        ...
        await _database.SeedAsync(ada);
        await _repository.AddForeignKeyAsync(ada.Id, "KaraFun", "remote-1", isEphemeral: true);
        await _repository.AddForeignKeyAsync(ada.Id, "KaraFun", "account-1", isEphemeral: false);
        await _repository.AddForeignKeyAsync(ada.Id, "YouTube", "remote-1", isEphemeral: true);

        Assert.Equal(1, await _repository.DeleteEphemeralForeignKeysAsync("KaraFun"));

        Assert.Null(await _repository.ReadByForeignKeyAsync("KaraFun", "remote-1"));
        Assert.NotNull(await _repository.ReadByForeignKeyAsync("KaraFun", "account-1"));
    ...
    public async Task RemoveForeignKey_FreesItForAnotherSinger()
    {
        ...
        await _repository.RemoveForeignKeyAsync(ada.Id, "KaraFun", "remote-1");
        await _repository.AddForeignKeyAsync(grace.Id, "KaraFun", "remote-1", isEphemeral: false);

        Assert.Equal("Grace", (await _repository.ReadByForeignKeyAsync("KaraFun", "remote-1"))?.Name);
    }
