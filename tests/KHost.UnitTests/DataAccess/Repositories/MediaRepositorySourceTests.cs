...
public class MediaRepositorySourceTests : IDisposable
{
    ...
    {
        var created = await _repository.CreateAsync(Row("/downloads/62794.khv", source: "Example"));

        var read = await _repository.ReadAsync(created.Id);

        Assert.Equal("Example", read?.Source);
    }
    ...
    {
        await _repository.CreateAsync(Row("/downloads/62794.khv", source: "Example"));
        ...
        Assert.Equal(new Dictionary<string, int> { ["Example"] = 1, ["YouTube"] = 1, [""] = 1 }, bySource);
    }
